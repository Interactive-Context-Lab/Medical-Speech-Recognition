...
class BatchRNN(nn.Module):
    def __init__(self, input_size, hidden_size, rnn_type=nn.LSTM, bidirectional=False, batch_norm=True, layernorm=False):
        super(BatchRNN, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.bidirectional = bidirectional
        if layernorm:
            self.batch_norm = nn.LayerNorm(input_size) if batch_norm else None
        else:
...
class DeepSpeech(nn.Module):
    ...
    def load_model(cls, path):
        ...
        model = cls(rnn_hidden_size=package['hidden_size'], nb_layers=package['hidden_layers'],
                    ...
                    rnn_type=supported_rnns[package['rnn_type']], bidirectional=package.get('bidirectional', True), layernorm=package.get('layernorm',False))
