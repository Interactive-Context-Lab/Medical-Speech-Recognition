...
class BeamCTCDecoder(Decoder):
    def __init__(self, labels, lm_path=None, alpha=0, beta=0, cutoff_top_n=40, cutoff_prob=1.0, beam_width=100,
                 num_processes=4, blank_index=0, prune = 0.00001):
        super(BeamCTCDecoder, self).__init__(labels)
        self._beam_width = beam_width
        self._prune = prune
        self._lm = kenlm.LanguageModel(lm_path) if not lm_path == None else None
    ...
    def prefix_beam_search(self, ctc, each_len, lm=None, k=10, alpha=0.30, beta=10, prune=0.00001):
        """
                ...
                for c in pruned_alphabet:
                    ...
                    # STEP 4: Extending with the end character
                        ...
                        if len(l) > 0 and c == l[-1]:
                            lm_prob = (10 ** lm.score(' '.join([a for a in l_plus]))) ** alpha if not lm == None else 1
                            Pnb[t][l_plus] += ctc[t][c_ix] * Pb[t - 1][l] * lm_prob
                            Pnb[t][l] += ctc[t][c_ix] * Pnb[t - 1][l]
                        # END: STEP 4

                        # STEP 5: Extending with any other non-blank character and LM constraints
                        elif len(l.replace(' ', '')) > 0 and c == ' ':
                            # lm_prob = lm.score(l_plus.strip(' ')) ** alpha
                            # Pnb[t][l_plus] += lm_prob * ctc[t][c_ix] * (Pb[t - 1][l] + Pnb[t - 1][l])
                            Pnb[t][l_plus] += ctc[t][c_ix] * (Pb[t - 1][l] + Pnb[t - 1][l])
                        else:
                            lm_prob = (10 ** lm.score(' '.join([a for a in l_plus]))) ** alpha if not lm == None else 1
                            Pnb[t][l_plus] += ctc[t][c_ix] * (Pb[t - 1][l] + Pnb[t - 1][l]) * lm_prob
                        # END: STEP 5
